...
internal sealed class SteamCmClient : IAsyncDisposable
{
    ...
    private async Task ConnectAsync(string endpoint, CancellationToken cancellationToken)
    {
        ...
        // A CM that never replies makes ConnectAsync hang for minutes, so each endpoint needs its own time limit;
        // on timeout throw TimeoutException (not OperationCanceledException),
        // so the retry loop in ConnectAndLogOnAsync moves on to the next CM.
    ...
    private void HandleNetMessage(byte[] packet)
    {
            ...
            _jobs.TryRemove(header.JobIdTarget.Value, out var job))
        {
            job.TrySetResult(new ServiceMethodResponse(
                // The proto defines optional int32 eresult = 13 [default = 2], so a missing value means Fail=2
                header.Result ?? 2,
                header.ErrorMessage,
                body));
            return;
    ...
    private static InvalidOperationException CreateLoggedOffException(byte[] body)
    {
        ...
        // Data["CmConflict"] is a language-neutral marker that CsLoadoutService uses to detect a session conflict.
        // Do not match on the localized Message text (it breaks in other languages).
