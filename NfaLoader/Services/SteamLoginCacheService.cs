...
internal sealed class SteamLoginCacheService
{
    ...
    // The cache file is a single process-wide resource (fixed path), so every read-modify-write must be serialized, or a background avatar refresh and
    // a delete or clear on the cached accounts page overwrite each other (losing a just-cached account / bringing a deleted one back). The critical sections are all
    // synchronous code (network fetches happen outside the lock), so a plain lock is enough.
    private static readonly object FileLock = new();

    // Cap on concurrent steamcommunity fetches per refresh, so opening too many connections at once does not trigger a 429 (same as AccountHistoryService).
    ...
    public async Task<int> RefreshProfilesAsync(IReadOnlyCollection<CachedSteamLoginAccount> accounts)
    {
        ...
        // Network fetches run concurrently outside the lock, throttled by a semaphore so too many steamcommunity connections at once do not trigger a 429.
        ...
        // Fetching can take seconds, and the user may delete or clear accounts meanwhile; so merge into the "current" disk state inside the lock,
        // instead of overwriting with the old snapshot read before fetching (which would bring deleted accounts back and lose concurrent writes).
        var updatedCount = 0;
        lock (FileLock)
        {
            var document = ReadDocument();
            foreach (var (account, profile, avatarPath) in results)
            {
                ...
                if (!string.IsNullOrWhiteSpace(profile.AvatarUrl))
                {
                    ...
                    else if (urlChanged)
                    {
                        ...
                        // or the old local image wins and the new avatar never shows.
                ...
                // Count only accounts that actually got profile data written: a fetch with all fields empty (nothing written) is not "synced", so the reported count is not inflated.
                if (wrote)
                {
                    updatedCount++;
                }
    ...
    private static async Task<SteamProfileData?> TryGetSteamProfileAsync(string steamId)
    {
        ...
        try
        {
            // The one-off nc parameter busts the edge cache: ?xml=1 is cached by the CDN for up to 1 hour (the cache key includes the query string, so a unique parameter always reaches the origin),
            // otherwise a refresh after a rename or avatar change fetches the old snapshot and "rolls back" the record to the old values.
        ...
        {
            // A missing or banned account returns 200 with an HTML error page, not XML.
            AppLog.Warn($"Steam profile response is not XML ({steamId}), skipped.");
            return null;
        }
    }
        ...
        string avatarUrl)
    {
        ...
        try
        {
            ...
            {
                // Reject a 200 + HTML "avatar" from a captive portal or hijacked network: once saved to disk, such a file always fails to decode in the UI.
                return null;
            }

            Directory.CreateDirectory(AvatarFolderPath);
            var avatarPath = Path.Combine(AvatarFolderPath, $"{GetSafeAvatarKey(steamId, accountName)}.jpg");
            // Atomic write: write a temp file, then replace the target in one step, so the UI thread never reads a half-written JPEG or hits a file mid-write.
            // The temp name has a random suffix: the post-login background refresh and a manual refresh can download the same account at once, and a fixed name would collide and fail silently.
